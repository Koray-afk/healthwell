  ...
  return (
    <Section id="features-appointments">
      <Container>
        ...
        <div className="mt-14 overflow-hidden rounded-[2.5rem] bg-[#1c1c1e] p-4 sm:p-6 lg:p-8">
          <motion.div
            initial="hidden"
            whileInView="show"
            viewport={viewport}
            ...
            {tiles.map((t, i) => {
              ...
              return (
                <motion.article
                  ...
                  className="group relative flex flex-col gap-4 overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#f5f5f7] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all hover:border-primary/30 hover:shadow-[0_8px_30px_rgba(59,130,246,0.12)]"
                >
                  {/* Hover glow */}
    ...
    <div className="rounded-2xl border border-black/[0.06] bg-white p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <span className="flex size-6 items-center justify-center rounded-lg text-xs text-muted-foreground hover:bg-black/5">‹</span>
          <span className="flex size-6 items-center justify-center rounded-lg text-xs text-muted-foreground hover:bg-black/5">›</span>
        </div>
      </div>
...
function VideoMock() {
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#e8f0fe] to-[#d4e4fc]">
      <div className="flex h-36 items-center justify-center">
        <div className="flex flex-col items-center gap-1">
          <div className="size-16 rounded-full bg-gradient-to-br from-[#f8b4b4] to-[#fcd6a7]" />
          <div className="mt-1 h-2 w-12 rounded bg-white/40" />
          <div className="h-1.5 w-20 rounded bg-white/30" />
        </div>
      </div>
      <div className="absolute left-2 top-2 flex items-center gap-1.5 rounded-full bg-black/50 px-2.5 py-1">
        <span className="size-2 animate-pulse rounded-full bg-red-500" />
        <span className="text-[10px] font-medium text-white">00:16:08</span>
      </div>
      <div className="flex items-center justify-center gap-3 border-t border-white/30 bg-white/60 px-4 py-2.5 backdrop-blur-sm">
...
function SearchMock() {
  const doctors = [
    { name: "Dr. James Lee", role: "Physical Therapist", color: "bg-blue-400" },
    { name: "Dr. Ava Patel", role: "Gastroenterologist", color: "bg-rose-400" },
    { name: "Dr. William A.", role: "Cardiologist", color: "bg-emerald-400" },
  ];
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {doctors.map((d) => (
          <div key={d.name} className="flex items-center gap-2.5 rounded-xl border border-black/[0.04] bg-white px-3 py-2">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end justify-end gap-2">
        <div>
          <div className="ml-auto max-w-[85%] rounded-2xl rounded-br-md bg-primary px-3.5 py-2.5 text-xs leading-relaxed text-white">
        ...
        <span className="mb-5 flex size-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-[9px] font-bold text-primary">Y</span>
      </div>
      <div className="flex items-end gap-2">
        <span className="mb-5 flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-[9px] font-bold text-emerald-600">D</span>
        <div>
          <div className="max-w-[85%] rounded-2xl rounded-bl-md bg-white px-3.5 py-2.5 text-xs leading-relaxed text-foreground shadow-sm ring-1 ring-black/[0.04]">
    ...
    <div className="flex flex-col items-center gap-2">
      {items.map((it, i) => (
        <div key={it.title} className="flex flex-col items-center">
          <div className="flex items-center gap-2.5 rounded-2xl border border-black/[0.06] bg-white px-4 py-3 shadow-sm">
